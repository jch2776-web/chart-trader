...
바이낸스 한도: 2,400 weight/분 (상한에 맞춤)`}</Formula>
  ...
  권장: 최소 진입 점수 70 이상 (RR≥2.0 + TP1 조건)`}</Formula>
        ...
        <H2>전략별 주요 차이점</H2>
        <Table
          headers={['항목', '기존 돌파', '리더-리테스트 v2']}
          rows={[
            ['진입 타이밍', '돌파 순간', '돌파 후 리테스트 확인봉 완성 후'],
            ['거래량 조건', '필수 (TRIGGERED/PENDING 구분)', '없음 (레벨 터치+재탈환 확인)'],
            ['추세 필터', '없음', '4H EMA20 > EMA50 (롱 전용, 기본 ON)'],
            ['추격 진입 방지 ②', '돌파선 이탈폭 체크 적용', '자동 무시 (개념 없음, UI 비활성)'],
            ['추격 진입 방지 ①③', '적용', '적용 (신호 경과 시간·주문 시점 이탈폭)'],
            ['최소봉/최대봉', '해당 없음', '기본 minBars=2, maxBars=8'],
            ['자동매매 방향', '양방향 (both)', '기본 롱만 (설정 가능)'],
            ['히스토리 배지', <Badge color="#3b8beb">ALT추천 + 돌파</Badge>, <Badge color="#9b59b6">ALT추천 + 리테스트</Badge>],
          ]}
        />
        ...
        <Card title="성과 분석과의 연동" accent="#3b8beb">
          <Ul items={[
            ...
            '거래 히스토리에 보라색 "ALT추천 + 리테스트" 배지 표시 (기존 돌파는 파란색 "ALT추천 + 돌파")',
