      ...
      <>
        ...
        <Card title="자동매매 동작 방식" accent="#f0b90b">
          <Ul items={[
            ...
            '1H 스캔 → 5초 대기 → 4H 스캔 → 5초 대기 → 1D 스캔',
            '스코어 90점 이상 심볼만 대상',
            '타임프레임별 상위 2개 심볼에 모의 진입',
            '진입 조건: 마진 $100, 레버리지 3x, ISOLATED 모드',
            'localStorage에 활성 상태 저장 (새로고침 후에도 유지)',
          ]} />
        </Card>

        <Card title="자동매매 스캔 속도 제한" accent="#848e9c">
          <P>바이낸스 API 과부하 방지를 위해 수동 스캔보다 보수적인 속도 제한을 적용합니다.</P>
          <Formula>{`동시 요청: 2개 (수동 스캔의 3개보다 낮음)
요청 간격: 600ms (수동 스캔의 350ms보다 느림)
예상 처리: ~17 weight/초 → ~1,020 weight/분
바이낸스 한도: 2,400 weight/분 (한도의 43% 수준)`}</Formula>
        </Card>
        ...
        <Card title="주의 사항" accent="#f6465d">
          <Ul items={[
            '자동매매는 모의 거래 전용 — 실전 주문은 접수하지 않음',
            '스캔 중 앱을 새로고침하면 진행 중인 스캔이 중단됨',
            '장시간 실행 시 다수의 모의 포지션이 누적될 수 있음 (거래 히스토리 확인 권장)',
    ...
    content: (
      <>
        ...
        <Q q="자동매매 활성화 시 바이낸스 API 차단이 발생할 수 있나요?">
          <p>자동매매는 동시 요청 2개, 요청 간격 600ms의 보수적인 속도 제한을 적용하여 분당 약 1,020 weight를 사용합니다. 바이낸스 한도(2,400 weight/분)의 43% 수준으로, 정상 운영 시 차단이 발생하지 않습니다. 단, 수동 스캔과 동시에 실행 중이라면 합산 부하가 증가할 수 있으므로 자동매매 활성 중에는 대용량 수동 스캔을 피해주세요.</p>
        </Q>
      ...
      <>
        <Table
          ...
          rows={[
            ...
            ['자동매매', '매 정각 백그라운드 스캔 후 90점+ 상위 2개 심볼에 자동 모의 진입하는 기능'],
