...
※ 돌파 전략(기본 90점)과 혼동 주의 — 90점으로 두면 후보 0개`}</Formula>
  ...
  권장: 최소 진입 점수 75 내외 (40~80점 범위, 90 이상이면 진입 없음)`}</Formula>
        ...
        <Card title="BB MTF Dip + Auto Rescue DCA ★ 4번째 전략" accent="#0ecb81">
          ...
          <Ul items={[
            '1h BB(20, 2σ) 하단 침범 확인 — 중기 과매도 컨텍스트',
            '15m BB(20, 2σ) 하단 침범 확인 — 단기 진입 시점',
            'prevPrev MA 필터: 3봉 전 종가 > SMA(50) — 추세 하방 코인 제외',
            '낙도 방지 게이트: 현재 종가 > MA × (1 − maxMaDropPct) — 자유낙하 코인 차단',
            'LIMIT 전용 진입: 현재 15m 종가에 지정가 주문',
            'TP1 = 15m BB 중심선 (SMA20), TP2 = 진입가 + 2R',
          ]} />
  ...
  권장: 최소 진입 점수 55 내외 (전략 전환 시 자동 설정됨)`}</Formula>
        ...
        <Table
          ...
          rows={[
            ...
            ['거래량 조건', '필수', '없음', '없음', '없음'],
            ['추세 필터', '없음', '4H EMA 필터', 'EMA72 방향', 'prevPrev MA + 낙도 방지'],
        ...
        <Ul items={[
          ...
          'BB MTF DCA 자동설정: BB 기간/배수, MA 기간, 낙도 허용%, SL ATR배수, 구출 단계/간격 조정 가능',
          '스캔 타임프레임: BB MTF DCA는 1h+15m을 내부적으로 항상 사용 (스캔 TF 설정과 무관)',
        ]} />
