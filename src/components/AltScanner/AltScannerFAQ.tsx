    ...
    content: (
      <>
        ...
        <Card title="기본 동작" accent="#f0b90b">
          <Ul items={[
            '최소 진입 점수 이상 심볼만 자동 진입 — 자동설정(⚙)에서 조정 가능 (기존 돌파 90점 · 리더-리테스트 65점 · FVG 75점 · BB MTF DCA 55점)',
            '스캔 전략 4종 선택 가능: 기존 돌파(breakout) / 리더-리테스트(leader-retest) / FVG POC + EMA72 / BB MTF DCA — 자동설정에서 독립 선택',
...
바이낸스 한도: 2,400 weight/분 (상한에 맞춤)`}</Formula>
  ...
  },
  {
    ...
    content: (
      <>
        <P>
          ALT추천 스캐너와 자동매매는 <strong style={{ color: '#f0b90b' }}>네 가지 스캔 전략</strong>을 지원합니다.
        ...
        <Card title="리더-리테스트 v2 (Leader Retest)" accent="#9b59b6">
          <P>
            돌파가 이미 발생한 SR 레벨로 가격이 되돌아와 <strong style={{ color: '#d1d4dc' }}>리테스트 확인봉(저점 터치 + 종가 재탈환)</strong>까지 완료된 구간을 포착합니다.
            단순 레벨 근접이 아닌 실제 수요 확인 후 진입하므로 기존 v1 대비 신호 품질이 향상됩니다.
          </P>
          <Ul items={[
  ...
  1) SR레벨 스코어 ≥ 20
  2) [n-maxBars ~ n-minBars] 봉에서 돌파봉 B 탐지
     B[-1].close < level,  B.close > level + ATR×0.1
  3) [B+1 ~ n-1] 봉에서 확인봉 C 탐지
...
정규화 재보정 후 실전 분포: 좋은 후보 60~75점대 (기존 50초반에서 상향)
기본 임계값 65점 — 전략 전환(자동설정) 시 minCandidateScore=65 자동 설정됨
※ 돌파 전략(기본 90점)과 혼동 주의 — 90점으로 두면 후보 0개`}</Formula>
          </Card>
        </Card>

        <Card title="FVG POC + EMA72" accent="#f0b90b">
          <P>
            FVG(Fair Value Gap) 구간의 POC(Point of Control)가 EMA72에 수렴했을 때 진입합니다.
            미충족 수요(FVG) 구간의 핵심 가격대와 중기 추세선이 겹치는 시점을 포착하는 전략입니다.
          </P>
          <Ul items={[
            'FVG 박스를 N개 bins으로 분할해 거래량 가중 POC 산출',
            'EMA72 기준 방향 필터: 롱은 EMA72 상향, 숏은 EMA72 하향',
            '추격 진입 방지 ② POC 이탈폭 체크 적용 (기존 돌파선 이탈폭과 동일 로직)',
            '자동매매에서도 완전 지원 (모의/실전 동일)',
          ]} />
          <Card title="점수 범위 (자동매매 최소 진입 점수 참고)" accent="#5e6673">
            <Formula>{`점수 = 40 + RR×15 + (TP1 있으면 +10)
  RR=1.5 → 62점 | RR=2.0 → 70점 | RR=2.6 → 79점
  권장: 최소 진입 점수 75 내외 (40~80점 범위, 90 이상이면 진입 없음)`}</Formula>
          </Card>
        </Card>

        <Card title="BB MTF Dip + Auto Rescue DCA ★ 4번째 전략" accent="#0ecb81">
          <P>
            1h 볼린저 밴드 하단 침범 + 15m 볼린저 밴드 하단 침범이 동시에 발생한 이중 과매도 구간에서
            LIMIT 딥 매수 진입합니다. 낙도 방지 필터(MA 대비 하락 제한)와 추세 판단 필터를 적용해
            무작위 낙도 코인을 걸러냅니다.
          </P>
          <Ul items={[
            '1h BB(20, 2σ) 하단 침범 확인 — breachLookback1h(기본 3)봉 중 하나 이상',
            '15m BB(20, 2σ) 하단 침범 확인 — 마지막 확정봉 기준',
            '신선한 침범 보호 (fresh-breach guard): 직전 breachLookback15m(기본 4)봉 중 1개 이상이 BB 하단 위 — 오래 갇혀있던 코인 제외',
            '낙도 방지 게이트: 현재 종가 > MA × (1 − maxMaDropPct) — 자유낙하 코인 차단',
            'LIMIT IOC 전용 진입: 현재 15m 종가에 즉시체결 시도, 미체결 시 자동취소',
            'TP1 = min(15m BB 중심선, 진입가 × 1.012) — BB중심선 또는 +1.2% 중 먼저 닿는 쪽',
            'TP2 = 진입가 × 1.025 (+2.5% 고정)',
            'SL = 진입가 − ATR × slAtr (기본 1.5배)',
            '롱 전용 — 숏(상방 BB 침범) 전략은 미구현',
          ]} />
          <Card title="구출 DCA (Auto Rescue DCA)" accent="#9b59b6">
            <P>
              진입 후 가격이 하락할 경우 미리 계획된 추가 매수 레벨에서 포지션을 늘려
              평균 단가를 낮추고 손익분기점 진입을 앞당깁니다.
            </P>
            <Ul items={[
              '구출 단계: 최대 2회 (설정에서 0~5회 조정 가능, 0=비활성)',
              '간격: 기본 ATR×1.0마다 한 단계 (설정에서 ATR 배수 조정 가능)',
              '최대 예산 배수: 기본 3.10× — 초기 포지션 포함 총 자금의 3.1배 이내',
              '스캔 결과에 구출 레벨이 표시됨 (미리보기)',
              '⚠ 구출 DCA 실행은 현재 수동 확인 필요 — 완전 자동화는 향후 업데이트 예정',
            ]} />
          </Card>
          <Card title="점수 범위 (자동매매 최소 진입 점수 참고)" accent="#5e6673">
            <Formula>{`점수 = 40 + min(30, 1h침범% × 500) + min(30, 15m침범% × 1000)
  예: 1h 0.3% 침범 + 15m 0.5% 침범 → 40+15+30 = 85점
  예: 1h 0.1% 침범 + 15m 0.1% 침범 → 40+5+10 = 55점
  권장: 최소 진입 점수 55 내외 (전략 전환 시 자동 설정됨)`}</Formula>
          </Card>
        </Card>

        <H2>전략별 주요 차이점</H2>
        <Table
          headers={['항목', '기존 돌파', '리더-리테스트 v2', 'FVG POC + EMA72', 'BB MTF DCA']}
          rows={[
            ['진입 타이밍', '돌파 순간', '돌파 후 리테스트 확인봉 완성 후', 'FVG 박스 POC 수렴 + EMA72 조건 성립 시', '1h+15m 이중 BB 하단 침범 확인 후'],
            ['방향', '양방향', '기본 롱', '설정 가능', '롱 전용 (딥 매수)'],
            ['거래량 조건', '필수', '없음', '없음', '없음'],
            ['추세 필터', '없음', '4H EMA 필터', 'EMA72 방향', '낙도 방지 게이트 + fresh-breach guard'],
            ['DCA', '없음', '없음', '없음', '구출 DCA 2단계 지원'],
            ['점수 범위', '0~100 (기본 90)', '40~84 (기본 65)', '40~84 (기본 75)', '40~100 (기본 55)'],
            ['히스토리 배지', <Badge color="#3b8beb">ALT추천 + 돌파</Badge>, <Badge color="#9b59b6">ALT추천 + 리테스트</Badge>, <Badge color="#f0b90b">FVG POC + EMA72</Badge>, <Badge color="#0ecb81">BB MTF DCA</Badge>],
          ]}
        />

        <H2>설정 위치</H2>
        <Ul items={[
          <><strong style={{ color: '#d1d4dc' }}>모달 수동 스캔</strong>: ALT추천 모달 상단 "전략" 버튼 (기존 돌파 / 리더-리테스트 / FVG POC + EMA72 / BB MTF DCA) — localStorage에 개별 저장</>,
          <><strong style={{ color: '#d1d4dc' }}>자동매매</strong>: 자동설정(⚙) &gt; 스캔 전략 — 모의/실전 각각 독립 저장</>,
          '두 설정은 서로 완전히 독립: 모달에서 FVG를 보고 있어도 자동매매 전략에 영향 없음',
          '리더-리테스트 자동설정: 최소봉/최대봉, 허용폭(ATR), 오버슈트(ATR), 방향, 4H 추세 필터 ON/OFF 조정 가능',
          'FVG 자동설정: POC 룩백봉 수, bins 수, EMA 기간, 유니버스 TopN, 방향 조정 가능',
          'BB MTF DCA 자동설정: BB 기간/배수, MA 기간, 낙도 허용%, SL ATR배수, 구출 단계/간격, TP1/TP2 고정% 조정 가능',
          'BB MTF DCA 스캔 TF: 내부적으로 항상 1h+15m 데이터를 fetch — 스캔 TF·진입허용 TF·스캔 주기를 모두 [15m]으로 설정 권장',
          'BB MTF DCA 타임스탑: 15m × 48봉 = 12시간 강제청산 권장 (자동설정 > 타임스탑 ON > 48봉)',
        ]} />

        <Card title="성과 분석과의 연동" accent="#3b8beb">
          <Ul items={[
            '기존 돌파 진입: altMeta.strategyId = "breakout" → 파란색 배지 "ALT추천 + 돌파"',
            '리더-리테스트 진입: altMeta.strategyId = "leader-retest" → 보라색 배지 "ALT추천 + 리테스트"',
            'FVG POC + EMA72 진입: altMeta.strategyId = "fvg-poc-ema72" → 금색 배지 "FVG POC + EMA72"',
            'BB MTF DCA 진입: altMeta.strategyId = "bb-mtf-dca" → 초록색 배지 "BB MTF DCA"',
            '성과 분석 "구간별 성과 보기" > 전략 탭에서 네 전략의 성과 직접 비교 가능',
          ]} />
        </Card>
      </>
    ),
        ...
        <Card title="① 실전 진입 로직 (전략별 분기)" accent="#0ecb81">
          ...
          <div style={{ marginTop: 8, padding: '6px 10px', background: 'rgba(91,156,246,0.08)', border: '1px solid rgba(91,156,246,0.25)', borderRadius: 5, fontSize: '0.78rem' }}>
            ...
            <ul style={{ margin: '4px 0 0 16px', padding: 0, lineHeight: 1.7 }}>
              ...
              <li><b>INVALID</b>: 진입 스킵 (구조 붕괴 / chaseThreshold 초과)</li>
            </ul>
  ...
  },
  {
    ...
    content: (
      <>
        ...
        <Q q="FVG POC + EMA72 전략은 모의/실전 자동매매에서도 사용할 수 있나요?">
          <p>네, 모의 자동매매와 실전 자동매매 모두 지원됩니다. 자동설정(⚙) &gt; 스캔 전략에서 "FVG POC + EMA72"를 선택하면 됩니다. 점수 범위가 40~80점이므로 최소 진입 점수를 75 내외로 설정하세요 (기본값 90으로 두면 진입 신호가 나오지 않습니다). 전략 전환 시 자동으로 권장 점수로 조정됩니다. 전략실험실에서도 동일하게 지원됩니다.</p>
        </Q>
        <Q q="FVG POC란 무엇인가요?">
          <p>FVG(Fair Value Gap)는 급격한 가격 움직임으로 인해 시장에서 채워지지 않은 수요/공급 구간입니다. POC(Point of Control)는 해당 FVG 구간을 N개 bins으로 분할했을 때 거래량이 가장 집중된 가격대를 의미합니다. FVG POC + EMA72 전략은 이 POC 가격이 EMA72(중기 추세선)와 수렴하는 시점을 포착해 진입합니다.</p>
        </Q>
  ...
  },
  {
    ...
    content: (
      <>
        <P>
          <strong style={{ color: '#f0b90b' }}>L2L (Level to Level)</strong> 은 현재 차트 캔들을 분석해
          리더-리테스트 전략의 <strong style={{ color: '#d1d4dc' }}>진입 구간 / 목표가 / 손절선</strong>을
          차트 위에 시각적으로 표시하는 보조 도구입니다.
          자동매매 실행이 아닌 <Badge color="#f0b90b">인간 판단 지원 전용</Badge>입니다.
        </P>

        <Card title="활성화 방법" accent="#f59e42">
          <Ul items={[
            '툴바의 ⊞ L2L 버튼 클릭 → 주황색으로 활성화',
            '다시 클릭하면 오버레이 제거',
            '종목 전환 시 새 캔들로 자동 재계산됨',
          ]} />
        </Card>
        ...
        <Table
          headers={['요소', '색상', '의미']}
          rows={[
            ['지지 구간 (S1~S6)', '초록 점선', '가격이 반등하는 지지 레벨 — 번호가 작을수록 더 강한 레벨'],
            ['저항 구간 (R1~R6)', '빨강 점선', '가격이 되돌리는 저항 레벨 — 번호가 작을수록 더 강한 레벨'],
            ['진입 구간 (밴드)', '초록/빨강 반투명 밴드', '리테스트 확인봉 기준 진입 허용 가격 범위'],
            ['이상적 진입가 (실선)', '초록/빨강 2px 실선', 'SR 플립 레벨 — 이 가격 근처가 최적 진입 타점'],
            ['목표가 TP1 (실선)', '파란색 2px 실선', '다음 SR 저항까지의 1차 목표가'],
            ['손절 SL (실선)', '주황/빨강 2px 실선', '리테스트 윅 기반 손절가 — 이탈 시 전략 무효'],
          ]}
        />

        <H2>우상단 요약 패널 설명</H2>
        <Card title="요약 패널 항목" accent="#3b8beb">
          <Table
            headers={['항목', '설명']}
            rows={[
              ['최종 판단', '가격·점수·비슷한 과거·큰 흐름을 합쳐 지금 할 일을 표시'],
              ['▲ 매수 / ▼ 매도', '감지된 방향. 매수는 상승 기대, 매도는 하락 기대'],
              ['신뢰도 N점', '닫힌 봉 기준 차트 전용 점수 (0~100). 실제 승률이 아닌 신호 품질 점수'],
              ['리테스트 레벨', 'SR 플립이 발생한 핵심 가격 레벨'],
              ['살/팔 가격대', '현재가가 이 범위 안에 있을 때만 진입 후보'],
              ['목표 가격', '먼저 수익 실현을 고려할 가격'],
              ['손절 가격', '이 가격을 지나면 손실 제한 또는 신호 폐기'],
              ['손익비', '잃을 수 있는 금액 대비 벌 수 있는 금액. 1 : 1.5 이상 권장'],
              ['점수 기준', '닫힌 봉으로 산정했는지와 원 전략 점수를 함께 표시'],
              ['큰 흐름', '상위 시간봉 L2L 방향이 현재 신호와 같은지 확인'],
              ['과거 전체', '같은 L2L 엔진의 최근 닫힌 봉 walk-forward 백테스트 전체 표본'],
              ['비슷한 과거', '현재 셋업과 같은 방향 + 현재 신뢰도 이상 + 손익비 1.5 이상 표본만 따로 집계'],
            ]}
          />
        </Card>

        <H2>S/R 번호 읽는 법</H2>
        <Card title="지지(S) / 저항(R) 레벨 번호" accent="#0ecb81">
          <P>
            차트 왼쪽에 표시되는 <strong style={{ color: '#0ecb81' }}>S1·S2·S3...</strong> 와
            <strong style={{ color: '#f6465d' }}> R1·R2·R3...</strong> 는
            SR 스코어가 높은 순서로 번호가 매겨집니다.
          </P>
          <Ul items={[
            <><Badge color="#0ecb81">S1</Badge> — 가장 강한 지지 레벨 (터치 횟수·최근성·스코어 종합 1위)</>,
            <><Badge color="#0ecb81">S2, S3...</Badge> — 그 다음 강한 지지 레벨 순서</>,
            <><Badge color="#f6465d">R1</Badge> — 가장 강한 저항 레벨</>,
            <><Badge color="#f6465d">R2, R3...</Badge> — 그 다음 강한 저항 레벨 순서</>,
            '최대 6개까지 표시 (S1~S6, R1~R6)',
            'ST(단기 60봉) / MT(중기 150봉) / LT(장기 300봉) 피벗 고점/저점을 군집 분석해 추출',
          ]} />
        </Card>

        <H2>계산 방식</H2>
        <Card title="L2L 내부 처리 순서" accent="#9b59b6">
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const, alignItems: 'center', marginBottom: 8 }}>
            {['캔들 데이터', '→', 'ATR(14) 계산', '→', 'SR 레벨 추출', '→', '리테스트 후보 탐색', '→', '스코어 산정', '→', '최고 점수 셋업 선택', '→', '진입/TP/SL 계산', '→', '차트 렌더링'].map((s, i) => (
              s === '→'
                ? <span key={i} style={{ color: '#2a2e39' }}>▶</span>
                : <span key={i} style={{ background: '#1a1e2a', border: '1px solid #2a2e39', borderRadius: 4, padding: '2px 8px', fontSize: '0.74rem', color: '#b2b8c4' }}>{s}</span>
            ))}
          ...
          <Ul items={[
            '미완성 봉을 제외하고 닫힌 봉만으로 SR 감지·리테스트 후보를 계산',
            '롱/숏 후보를 모두 평가 후 보정 신뢰도 1위 셋업 하나만 표시 (혼란 방지)',
            '보정 신뢰도 = 돌파 강도 + 리테스트 품질 + SR 강도 + R/R + 신선도를 반영한 차트 전용 점수',
            '최종 판단 = 가격 상태 + 점수 + 손익비 + 비슷한 과거 + 상위 시간봉 방향을 통합',
            '과거 전체 = 최근 닫힌 봉에서 같은 탐지 로직을 반복 적용한 간단 walk-forward R 백테스트',
            '비슷한 과거 = 현재 셋업과 같은 방향, 현재 신뢰도 이상, 손익비 1.5 이상인 과거 표본만 별도 집계',
            '셋업이 없으면 "현재 차트에서 유효한 리테스트 셋업 없음" 표시',
            'idealEntry = SR 플립 레벨 (돌파 확인된 SR 레벨 중심)',
            'hardStop = 리테스트 윅 극단값 ± ATR × 0.10 버퍼',
            'tp1 = 이상적 진입가 방향의 첫 번째 강한 SR 레벨 (없으면 ATR × 2.0 fallback)',
          ]} />
        </Card>

        <H2>활용 방법</H2>
        <Ul items={[
          '먼저 최종 판단을 확인 — 진입 후보가 아니면 기다리거나 보류',
          '손익비 1.5 이상, 신뢰도 55점 이상, 비슷한 과거 평균 R이 양수인 셋업만 진입 고려',
          '큰 흐름이 반대라고 나오면 하위봉 신호가 좋아도 진입 보류',
          '목표 가격 근처에서 이미 저항을 받고 있다면 진입 보류',
          '손절 가격을 지난 상태면 해당 신호는 무효 — 다음 봉 확인 후 재분석',
          '4H / 1D 차트에서 먼저 L2L 방향을 확인하고 하위 봉에서 진입 타점 찾기',
        ]} />

        <Card title="주의 사항" accent="#f6465d">
          <Ul items={[
            'L2L은 참고 지표일 뿐 — 자동매매 주문을 발생시키지 않음',
            '짧은 봉(1m·3m)에서는 노이즈가 많아 신뢰도 낮을 수 있음 (15m 이상 권장)',
            'SR 레벨은 최근 캔들 기준으로 실시간 갱신됨 — 같은 종목이라도 봉 단위 변경 시 다르게 계산됨',
            '최종 판단과 과거 검증은 참고값이며 미래 수익이나 승률을 보장하지 않음',
            '신뢰도 점수가 낮거나 셋업이 없으면 현재 구조에서 진입 근거 부족을 의미',
          ]} />
        </Card>
      </>
    ),
    ...
    content: (
      <>
        <div style={{
          ...
          fontSize: '0.8rem', color: '#f0b90b',
        }}>
          ⚡ 스캘핑은 ALT추천 스캐너와 <strong>완전히 독립된 별개 시스템</strong>입니다.
          툴바의 <Badge color="#f0b90b">⚡ 스캘핑FAQ</Badge> 버튼에서 전체 가이드를 확인하세요.
        </div>

        <P>
          스캘핑 자동매매는 <strong style={{ color: '#d1d4dc' }}>호가창 미시구조(microstructure)</strong>를 실시간으로
          분석해 초단기(수초~수분) 진입 신호를 포착하고, 메이커 GTX 지정가 주문으로 자동 진입·익절·손절합니다.
        </P>

        <H2>ALT추천 vs 스캘핑 핵심 차이</H2>
        <Table
          headers={['항목', 'ALT추천 (스윙)', '스캘핑']}
          rows={[
            ['분석 대상', '봉 마감 캔들 데이터', '실시간 호가 + 체결 스트림'],
            ['진입 타이밍', '봉 마감 후 SR·추세선 분석', '밀리초 단위 호가 불균형·체결 압력'],
            ['주문 유형', 'MARKET (즉시 체결)', 'LIMIT GTX (메이커 수수료 -0.02%)'],
            ['보유 시간', '수분~수일', '수초~수분'],
            ['신호 기반', '추세선 돌파 / SR 리테스트 / FVG / BB딥', '마이크로 모멘텀 / 마이크로 반전'],
            ['전략 설정', '자동설정(⚙) > 스캔 전략', '스캘핑설정(⚙) > signalMode'],
            ['FAQ', '현재 문서 (알트FAQ)', '툴바 ⚡ 스캘핑FAQ'],
          ]}
        />

        <H2>스캘핑 신호 유형 (2종)</H2>
        <Card title="① 마이크로 모멘텀 (micro-momentum)" accent="#0ecb81">
          <P>호가 불균형과 체결 압력이 <strong style={{ color: '#0ecb81' }}>같은 방향</strong>으로 기준치를 초과할 때 진입합니다.</P>
          <Ul items={[
            '롱: bookImbalance > +0.20 AND tradePressure > +0.20 (기본값)',
            '숏: bookImbalance < -0.20 AND tradePressure < -0.20',
            'TTL 3초 안에 GTX 체결 안 되면 리프라이스 → 최대 2회 재시도',
          ]} />
        </Card>
        <Card title="② 마이크로 반전 (micro-revert)" accent="#f6465d">
          <P>체결 압력이 <strong style={{ color: '#f0b90b' }}>극단적</strong>이면서 호가 불균형이 <strong style={{ color: '#f6465d' }}>반대 방향</strong>일 때 페이드(역추세) 진입합니다.</P>
          <Ul items={[
            '예: tradePressure > +0.55 (극단 매수) AND bookImbalance < 0 (호가 매도 우위) → 숏 진입',
            'TTL 2초 (모멘텀보다 짧음)',
            'signalMode: "momentum" / "revert" / "both" — 스캘핑설정에서 선택',
          ]} />
        </Card>

        <H2>리스크 게이트 (8단계)</H2>
        <P>모든 진입 신호는 아래 게이트를 순서대로 통과해야 합니다. 하나라도 실패하면 해당 심볼 진입 차단.</P>
        <Table
          headers={['게이트', '기본값', '설명']}
          rows={[
            ['연속 손절 차단기', '3회', '연속 N회 손실 시 수동 해제 전까지 전체 정지'],
            ['일일 최대 거래 수', '50회', '세션당 진입 총 횟수 한도'],
            ['최대 노출 한도', '$100', '열린 포지션 합산 명목가치 상한'],
            ['심볼 쿨다운', '30s', '거래 종료 후 동일 심볼 재진입 대기'],
            ['스프레드 게이트', '3 bps', '스프레드 초과 시 차단'],
            ['뎁스 게이트', '$50,000', '10bps 내 호가 잔량 USD 기준 미달 시 차단'],
            ['지연 게이트', '500 ms', '시장 데이터 신선도 초과 시 차단'],
            ['신호 점수', '0.25', '점수 0.25 미만 자동 필터'],
          ]}
        />

        <H2>TP / SL 구조</H2>
        <Card title="TP/SL 가격 계산 요약" accent="#3b8beb">
          <Ul items={[
            'stopDist = max(halfSpread × 3.0, 진입가 × 0.001)  [최소 10 bps]',
            'SL = 진입가 - stopDist (롱) / + stopDist (숏)',
            'TP = 진입가 + max(stopDist × 1.5, 수수료 커버 + 최소 ROI)',
            'TP/SL 누락 시 15초마다 자동 재부착',
            'SL 등록 실패 시 클라이언트사이드 mark price 감시 → MARKET 폴백 청산',
          ]} />
        </Card>

        <Card title="주의 사항" accent="#f6465d">
          <Ul items={[
            'ALT추천 자동매매와 스캘핑을 동시에 활성화하면 API 요청이 합산됨 — weight 한도 주의',
            '스캘핑 전용 심볼은 스캘핑설정에서 별도 지정 (ALT스캐너 유니버스와 독립)',
            '전체 세부 설정·문제 해결은 툴바의 ⚡ 스캘핑FAQ 참조',
          ]} />
        </Card>
      </>
    ),
      ...
      <>
        <Table
          ...
          rows={[
            ...
            ['FVG', 'Fair Value Gap — 급격한 가격 이동으로 생긴 미충족 수요/공급 구간'],
            ['POC', 'Point of Control — FVG 구간 내 거래량이 가장 집중된 핵심 가격대'],
            ['EMA72', '72봉 지수이동평균. FVG POC + EMA72 전략의 방향 필터로 사용'],
            ['fvg-poc-ema72', 'FVG POC + EMA72 전략의 strategyId 식별자'],
          ]}
        />
      </>
    ),
