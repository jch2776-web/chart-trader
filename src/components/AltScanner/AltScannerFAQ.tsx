    ...
    content: (
      <>
        ...
        <Card title="기본 동작" accent="#f0b90b">
          <Ul items={[
            '최소 진입 점수 이상 심볼만 자동 진입 — 자동설정(⚙)에서 조정 가능 (기존 돌파 기본 90점 · 리더-리테스트 70점 · FVG 75점)',
            '스캔 전략 3종 선택 가능: 기존 돌파(breakout) / 리더-리테스트(leader-retest) / FVG POC + EMA72 — 자동설정에서 독립 선택',
...
※ 돌파 전략(기본 90점)과 혼동 주의 — 90점으로 두면 후보 0개`}</Formula>
  ...
  권장: 최소 진입 점수 75 내외 (40~80점 범위, 90 이상이면 진입 없음)`}</Formula>
        ...
        <H2>전략별 주요 차이점</H2>
        <Table
          headers={['항목', '기존 돌파', '리더-리테스트 v2', 'FVG POC + EMA72']}
          rows={[
            ['진입 타이밍', '돌파 순간', '돌파 후 리테스트 확인봉 완성 후', 'FVG 박스 POC 수렴 + EMA72 조건 성립 시'],
            ['거래량 조건', '필수 (TRIGGERED/PENDING 구분)', '없음 (레벨 터치+재탈환 확인)', '없음 (POC 가격 수렴 확인)'],
            ['추세 필터', '없음', '4H EMA20 > EMA50 (롱 전용, 기본 ON)', 'EMA72 방향 필터 내장'],
        ...
        <H2>설정 위치</H2>
        <Ul items={[
          <><strong style={{ color: '#d1d4dc' }}>모달 수동 스캔</strong>: ALT추천 모달 상단 "전략" 버튼 (기존 돌파 / 리더-리테스트 / FVG POC + EMA72) — localStorage에 개별 저장</>,
          ...
          '스캔 타임프레임: 1H 스캔으로도 4H 리테스트 패턴 감지 가능 (4H 추세 필터는 TF와 무관하게 4H 캔들 별도 조회)',
        ]} />
        ...
        <Card title="성과 분석과의 연동" accent="#3b8beb">
          <Ul items={[
            ...
            '성과 분석 "구간별 성과 보기" > 전략 탭에서 세 전략의 성과 직접 비교 가능',
          ]} />
        </Card>
      </>
    ),
